...
class BottleController extends GetxController {
  ...
  final RxList<BottleCardModel> hotBottles = <BottleCardModel>[].obs;
  ...
  Future<void> loadWeekHotBottles() async {
    ...
    try {
      isLoadingHotBottles.value = true;
      final response = await _apiService.getHotBottles(10, 1, timeRange: 'week');
      if (response.success && response.data != null) {
        hotBottles.assignAll(response.data!.bottles);
      }
    } finally {
  ...
  Future<void> loadMonthHotBottles() async {
    ...
    try {
      isLoadingHotBottles.value = true;
      final response = await _apiService.getHotBottles(10, 1, timeRange: 'month');
      if (response.success && response.data != null) {
        hotBottles.assignAll(response.data!.bottles);
      }
    } finally {
